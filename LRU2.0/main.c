...
void *thread_fun(void *ThreadID){
    ...
    if (req_page == NULL) {
        ...
        req_page = allocate_node(q, hash, random, tid);  
    ...
    if (access_node(q,req_page,random, tid)== -1) {
        req_page = allocate_node(q, hash, random, tid);
        // Add page entry to hash
        pthread_spin_lock(&hashtbl_lock);
        ...
        pthread_spin_unlock(&hashtbl_lock);
        access_node(q,req_page,random, tid);
    }
                
    sleep(pow(5.0, -3));
    access_done(q,hash,req_page, tid); 
...
int main() {
   ...
   for( int i = 0; i < THREADS; i++ ) {
    ...
    access_done_time_avg = access_done_time_avg/1024;
    ...
    printf("\n Average time taken for access_done : %d\n", access_done_time_avg);
    return SUCCESS;
}